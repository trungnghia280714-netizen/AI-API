...
app = FastAPI(title="INTELIGENT API", version="4.0")
...
# ============================================
# CONFIG - API KEYS CỦA BẠN
# ============================================

GROQ_API_KEY = "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"
POLLINATIONS_API_KEY = "sk_AqsTyfJPD4VZaDV7Vu88UlPMqtQQ3U6o"
HUGGINGFACE_API_KEY = "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"
...
# ============================================
# 3. TẠO VIDEO - HuggingFace
# ============================================

async def generate_video(prompt):
    try:
        async with httpx.AsyncClient(timeout=120.0) as client:
            # Gọi HuggingFace API tạo video
            response = await client.post(
            ...
            if response.status_code == 200:
                ...
                # Thử model khác nếu model đầu bị lỗi
                response = await client.post(
...
<div class="container">
    ...
    <div class="subtitle">
        ...
        <span class="badge">🚀 Chạy trên Render</span>
        <span class="badge">⚡ Hoàn toàn miễn phí</span>
    </div>
    ...
    <div class="status-bar">
        <span>🟢 INTELIGENT đang hoạt động</span>
...
async function generateImage() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            result.innerHTML = '❌ Lỗi: ' + (err.detail || 'Không tạo được ảnh');
        }
    } catch(e) {
        result.innerHTML = '❌ Lỗi: ' + e.message;
    }
...
async function generateVideo() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            const err = await res.json();
            result.innerHTML = '❌ Lỗi: ' + (err.detail || 'Không tạo được video');
        }
    } catch(e) {
        result.innerHTML = '❌ Lỗi: ' + e.message;
    }
